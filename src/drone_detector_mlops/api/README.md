...
1. **Model Loading**: On startup, the API loads `models/model.pth` into memory
2. **Image Processing**: Uploaded images are transformed using the same preprocessing as training
3. **Inference**: Model predicts drone or bird with confidence scores
...
- **Local mode** (`MODE=local`): Loads from `models/model.pth`
- **Cloud mode** (`MODE=cloud`): Loads from GCS bucket
